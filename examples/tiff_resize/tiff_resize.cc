...
main(int argc, char **argv)
{
  ...
  if (argc < 3) {
    std::cout << "tiff_resize input.tiff output.tiff resize_factor (image_idx)" << std::endl;
  ...
  const tinydng::DNGImage &image = images[image_idx];
  {
    ...
    const size_t step = size_t(resize_factor);
    const size_t dst_width = width / size_t(resize_factor);
    const size_t dst_height = height / size_t(resize_factor);
